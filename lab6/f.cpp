...
struct Heap {
    ...
    void insert(Student val) {
        heap.push_back(val);
        int i = heap.size()-1;
        while(i != 0 and heap[i].main_check(heap[parent(i)])) {
    ...
    void heapify(int i) {
        ...
        int r = right(i);
        int smallest = i;
        if(l < heap.size() and heap[l].main_check(heap[smallest])) smallest = l;
        if(r < heap.size() and heap[r].main_check(heap[smallest])) smallest = r;
        if(smallest != i) {
